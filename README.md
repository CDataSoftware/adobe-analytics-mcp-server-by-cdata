...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Adobe Analytics (beta)](https://www.cdata.com/download/download.aspx?sku=SCZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Adobe Analytics supported by the [CData JDBC Driver for Adobe Analytics](https://www.cdata.com/drivers/adobeanalytics/jdbc).
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/adobeanalytics/download/jdbc](https://www.cdata.com/drivers/adobeanalytics/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Adobe Analytics\`
...
5. Create a `.prp` file for your JDBC connection (e.g. `adobe-analytics.prp`) using the following properties and format:
